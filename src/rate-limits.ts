...
export function normalizeRateLimits(
  ...
  limitIds: string[] = ["codex"],
): NormalizedRateLimit[] {
  const allBuckets =
    response.rateLimitsByLimitId && Object.keys(response.rateLimitsByLimitId).length
...
export function isHardStop(reason: GuardReason): boolean {
  return (
...
function normalizeSnapshot(fallbackId: string, snapshot: RateLimitSnapshot): NormalizedRateLimit {
  const windows = [snapshot.primary, snapshot.secondary].filter(Boolean) as NonNullable<
...
function evaluateUsageLimits(
  rateLimits: NormalizedRateLimit[],
  policy: QuotaPolicy,
): "healthy" | "warning" | "pause_required" {
  ...
  if (
    usageWindows.some(
      ({ usedPercent, limitPercent }) => usedPercent >= Math.max(0, limitPercent - 5),
    )
  ) {
    return "warning";
  }
...
function isRestartAvailable(rateLimits: NormalizedRateLimit[], policy: QuotaPolicy): boolean {
  return usageLimitWindows(rateLimits, policy).every(
    ({ usedPercent, limitPercent }) => usedPercent < limitPercent,
  );
}

function usageLimitWindows(
  rateLimits: NormalizedRateLimit[],
  policy: QuotaPolicy,
): { usedPercent: number; limitPercent: number }[] {
  const usageLimits = policy.usageLimits;
  return rateLimits.flatMap((rateLimit) => {
    const windows: { usedPercent: number; limitPercent: number }[] = [];
...
function findFiveHourWindow(rateLimit: NormalizedRateLimit): RateLimitWindow | null {
  const windows = [rateLimit.primary, rateLimit.secondary].filter(Boolean) as RateLimitWindow[];
...
function fallbackWindow(rateLimit: NormalizedRateLimit): RateLimitWindow {
  return {
...
function hardStopReason(rateLimits: NormalizedRateLimit[]): GuardReason | null {
  for (const limit of rateLimits) {
    if (limit.credits && !limit.credits.unlimited && !limit.credits.hasCredits) {
      return "credits_depleted";
    }
    if (
      limit.rateLimitReachedType?.includes("credits_depleted") ||
...
function nextResetIso(rateLimits: NormalizedRateLimit[]): string | null {
