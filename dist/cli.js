...
import { blockForConfigurationRequired, checkStatus, createCheckpoint, monitor, monitorOnce, pauseGoal, resumeGoal, } from "./guard.js";
async function main() {
    ...
    switch (options.command) {
        case "configure-goal": {
            ...
            try {
                ...
                if (error instanceof IncompletePolicyConfigurationError) {
                    print(await blockForConfigurationRequired({ cwd: options.cwd, threadId, turnId: options.turnId }, error.message));
                    return;
                }
                throw error;
            }
        ...
        case "block-unconfigured-goal": {
            requireThreadId(threadId);
            print(await blockForConfigurationRequired({ cwd: options.cwd, threadId, turnId: options.turnId }, options.summary ??
        ...
        case "create-checkpoint": {
            requireThreadId(threadId);
            print(await createCheckpoint({ cwd: options.cwd, threadId, turnId: options.turnId }, options.reason ?? "unknown", options.summary ?? "No progress summary supplied."));
            return;
        }
        case "pause-goal": {
            requireThreadId(threadId);
            print(await pauseGoal({ cwd: options.cwd, threadId, turnId: options.turnId }, options.reason ?? "threshold_reached", options.summary ??
                "QuotaKeeper paused this Goal because the configured quota policy was reached."));
            return;
        }
        case "resume-goal": {
            requireThreadId(threadId);
            print(await resumeGoal({ cwd: options.cwd, threadId, turnId: options.turnId }, options.force));
            return;
        }
        case "monitor": {
            requireThreadId(threadId);
            if (options.once) {
                print(await monitorOnce({
                    cwd: options.cwd,
                    threadId,
                    turnId: options.turnId,
                }));
            ...
            await monitor({ cwd: options.cwd, threadId, turnId: options.turnId }, options.pollIntervalSeconds ?? 60);
            return;
        }
        default:
    ...
    for (let index = 0; index < rest.length; index += 1) {
        ...
        switch (arg) {
            ...
            case "--limit-ids":
                ...
                break;
            case "--reason":
                options.reason = requireValue(arg, value);
                index += 1;
                break;
            case "--summary":
